...
public class exam {
    ...
    public static String encrypt(String plaintext) {
        ...
        for (int i = 0; i < plaintext.length(); i++) {
            char character_check = plaintext.charAt(i); // Use a temprary variable (character_check) for identification
                                                        // of character type
    ...
    public static String decrypt(String ciphertext) {
        ...
        for (int i = 0; i < ciphertext.length(); i++) {
            char character_check = ciphertext.charAt(i); // Use a temprary variable (character_check) for identification
                                                         // of character type
            if (Character.isUpperCase(character_check)) {
                int k3 = ciphertext.charAt(i) - 'A'; // Use k3 variable to hold value of characters in integer
                int decrypted = ((k3 - k2 + 26) * k1_inverse) % 26; // Formula according to question "P = ((C − k2) ×
                                                                    // k1^-1) mod 26"
